...
namespace Biden.Func
{
    ...
    class Macro
    {
        ...
        private static void rr2()
        {
            ...
            sendkey("z");
            /*
            curColorLeft = GetColorAt(92, 271);
            curColorLeft2 = GetColorAt(25, 263);
            curColorRight = GetColorAt(248, 271);

            if ((curColorLeft.R == 255 && curColorLeft.G == 255) || (curColorLeft2.R == 255 && curColorLeft2.G == 255))
            {
                sendkey("{RIGHT 50}");
                LR = "R";
            }
            else if (curColorLeft.R == 255 && curColorLeft.G == 255)
            {
                sendkey("{LEFT 50}");
                LR = "L";
            }
            */
            int moveDistance = randomNum.Next(10, 160);

            int mobDist = 190;

            if (LR == "L")
            {
                sendkey("{LEFT " + moveDistance + "}");

                if (lastPosX < 170)
                {
                    for (int i = 20; i < mobDist; i = i + 3)
                    {
                        Color tempColor = GetColorAt(960 - i, 747);
                        // 154, 137,121
                ...
                {
                    int curLargeX = (int)(lastPosX * 6.53);
                    for (int i = 20; i < mobDist; i = i + 3)
                    {
                        int curX = curLargeX - i;
                        if (curX >= 1919)
                        {
                            curX = 1919;
                        }
                        Color tempColor = GetColorAt(curX, 747);
                        // 154, 137,121
                        if ((tempColor.R == 101 && tempColor.G == 51 && tempColor.B == 35) ||
                            (tempColor.R == 99 && tempColor.G == 84 && tempColor.B == 68) ||
                            (tempColor.R == 104 && tempColor.G == 89 && tempColor.B == 66) ||
        ...
        private static void getCurPos(CancellationTokenSource ct)
        {
        ...
        private static void getCurPos2(CancellationTokenSource ct)
        {
            Color curColorLeft2 = new Color();
        ...
        private static void rr3()
        {
            try
            {
                ...
                if (attackFlag)
                {
                    ...
                    Thread.Sleep(100);
                    User32.API.keybd_event(0XA2, 0, 0, 0); // 'A' key down
                    Thread.Sleep(100);
                    User32.API.keybd_event(0XA2, 0, KEYEVENTF_KEYUP, 0); // 'A' key up
                    Thread.Sleep(600);
                    User32.API.keybd_event(0XA2, 0, 0, 0); // 'A' key down
                    Thread.Sleep(100);
                    User32.API.keybd_event(0XA2, 0, KEYEVENTF_KEYUP, 0); // 'A' key up
                    ...
                    {
                        Thread.Sleep(500);
                        User32.API.keybd_event(0X23, 0, 0, 0); // 'A' key up
                        Thread.Sleep(100);
                        User32.API.keybd_event(0X23, 0, KEYEVENTF_KEYUP, 0); // 'A' key up
                        Thread.Sleep(100);
                    }
                }
                ...
                {
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X24, 0, 0, 0); // 'A' key up
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X24, 0, KEYEVENTF_KEYUP, 0); // 'A' key up
                    Thread.Sleep(50);
                }
                ...
                {
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X2D, 0, 0, 0); // 'A' key up
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X2D, 0, KEYEVENTF_KEYUP, 0); // 'A' key up
                    Thread.Sleep(50);
                }
        ...
        private static void rr4()
        {
            try
            {
                ...
                {
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X2D, 0, 0, 0); // insert
                    Thread.Sleep(200);
                    User32.API.keybd_event(0X2D, 0, KEYEVENTF_KEYUP, 0); // insert
                    Thread.Sleep(50);
                }
